...
trigger:"hover focus",title:"",delay:0,html:false,container:false};$.fn.tooltip.noConflict=function(){$.fn.tooltip=old;return this}}(window.jQuery);!function($){var Affix=function(element,options){this.options=$.extend({},$.fn.affix.defaults,options);this.$window=$(window).on("scroll.affix.data-api",$.proxy(this.checkPosition,this)).on("click.affix.data-api",$.proxy(function(){setTimeout($.proxy(this.checkPosition,this),1)},this));this.$element=$(element);this.checkPosition()};Affix.prototype.checkPosition=function(){if(!this.$element.is(":visible"))return;var scrollHeight=$(document).height(),scrollTop=this.$window.scrollTop(),position=this.$element.offset(),
...
toggle,Dropdown.prototype.toggle).on("keydown.dropdown.data-api",toggle+", [role=menu]",Dropdown.prototype.keydown)}(window.jQuery);!function($){var Modal=function(element,options){this.options=options;this.$element=$(element).delegate('[data-dismiss="modal"]',"click.dismiss.modal",$.proxy(this.hide,this));this.options.remote&&this.$element.find(".modal-body").load(this.options.remote)};Modal.prototype={constructor:Modal,toggle:function(){return this[!this.isShown?"show":"hide"]()},show:function(){var that=this,e=$.Event("show");this.$element.trigger(e);if(this.isShown||e.isDefaultPrevented())return;this.isShown=true;this.escape();
...
(function($){$.extend({"jshadow":function(definition){if(typeof definition.trigger==="undefined"||definition.trigger===null)definition.trigger="click";definition=$.extend(true,{},{"trigger":"click","binding":$.jshadow.default_binding,"routines":{}},definition);$.fn[definition.name]=function(options){var settings=$.extend(true,{},definition.defaults,options);$(this).each(function(){var conf=$.extend(true,{},{"ns":""},settings),def=$.extend(true,{},definition),wrapper=$(this),data_value;$.each(conf,
function(key,value){data_value=wrapper.data(def.name+"-"+key);if(typeof data_value!=="undefined")conf[key]=data_value});def.api={};$.each(def.routines,function(key,func){def.api[key]=function(){$.proxy(func,wrapper)(conf,wrapper,def)}});definition.binding(conf,wrapper,def)})};$(function(){$.jshadow.register(definition);$(definition.wrapper).not('[data-api="off"], [data-api-'+definition.name+'="off"]')[definition.name]()})}});var register=[];$.jshadow.register=function(def){register.push(def)};$.fn.jshadow=
function(){$(this).each(function(){var self=$(this);$.each(register,function(i,def){self.find(def.wrapper).not('[data-api="off"], [data-api-'+def.name+'="off"]')[def.name]()})})};$.jshadow.children=function(conf,wrapper,def,selector){var selection=wrapper.find(selector).not(wrapper.find(def.wrapper+" "+selector)).not("[data-"+def.name+"-ns]");if(conf.ns!=="")selection=selection.add(wrapper.find("[data-"+def.name+'-ns="'+conf.ns+'"]'));return selection};$.jshadow.targets=function(conf,wrapper,def){var selection=
wrapper.find(def.defaults["api-target"]).not(wrapper.find(def.wrapper+" "+def.defaults["api-target"])).not("[data-"+def.name+"-ns]");if(conf.ns!=="")selection=selection.add(wrapper.find("[data-"+def.name+'-ns="'+conf.ns+'"]'));return selection};$.jshadow.default_binding=function(conf,wrapper,definition){var elements;if(typeof definition.action!=="undefined"){elements=$.jshadow.children(conf,wrapper,definition,conf["api-target"]);elements.on(definition.trigger+"."+definition.name,function(event){$.proxy(definition.action,
this)(event,conf,wrapper,definition)})}$.each(definition.routines,function(key,func){wrapper.on(key+"."+definition.name,func)});if(typeof definition.init!=="undefined")definition.init(conf,wrapper,definition)}})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"backeselect","wrapper":"[data-backselect-context]","defaults":{"api-source":"[data-backselect-source]","api-backup":"[data-backselect]","fadein":"slow","fadeout":"slow"},"init":function(conf,wrapper,self){var $sources=$.jshadow.children(conf,wrapper,self,conf["api-source"]),$backups=$.jshadow.children(conf,wrapper,self,conf["api-backup"]);var source_attr=conf["api-source"].replace(/(^\[|\]$)/gi,"");$sources.on("change.mjolnir_backselect",function(event){var blankstate=
true;$sources.each(function(){var $source=$(this);if($source.is('[type="checkbox"]')){if(Boolean($source.attr(source_attr))!==$source.is(":checked")){blankstate=false;return false}}else if($source.attr(source_attr)!==$source.val()){blankstate=false;return false}});if(blankstate){$backups.fadeIn(conf.fadein).closest(".composite").find(".control-label .off").fadeIn(conf.fadein);$backups.filter("select").each(function(){var select2=$(this).siblings(".select2-container");if(typeof select2!=="undefined"){$(this).hide();
select2.fadeIn(conf.fadein)}})}else{$backups.each(function(){var $context=$(this),$inputs=$("input, textarea, select",$context).not('[type="submit"]');$inputs.each(function(){var $input=$(this),blank=$input.attr("data-backselect-blank");if(typeof blank!=="undefined")$input.val(blank);else $input.val("")})});$backups.fadeOut(conf.fadeout).closest(".composite").find(".control-label .off").fadeOut(conf.fadeout);$backups.siblings(".select2-container").fadeOut(conf.fadeout)}});if($sources.length>0)$($sources[0]).trigger("change")}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($,undefined){$.jshadow({"name":"equation","wrapper":"[data-equation-context]","defaults":{"api-target":"[data-equation]"},"init":function(conf,$wrapper,self){var $equations=$.jshadow.children(conf,$wrapper,self,conf["api-target"]);var addslashes=function(str){return(str+"").replace(/([\\"'])/g,"\\$1").replace(/\0/g,"\\0")};$equations.each(function(){var $equation=$(this),formula=$equation.attr("data-equation"),context_variables=null;context_variables=formula.match(new RegExp("\\$[a-zA-Z0-9_]+",
"gi"));var update=function(){var field_value,__return;for(var i=0;i<context_variables.length;++i){var $field=$('[name="'+context_variables[i].replace("$","")+'"]',$wrapper);field_value=$field.val();if(field_value===undefined){console.log("equation.shadow -- unable to locate "+context_variables[i]);return""}if($field.attr("data-equation-type")=="literal")field_value='"'+addslashes(field_value)+'"';else if(!field_value.match("[0-9.]+"))field_value=0;eval("var "+context_variables[i]+"="+field_value+
";")}try{eval("__return = "+formula+";");if(isNaN(__return))return"";return __return}catch(e){console.log("equation.shadow -- failed to compute [ "+formula+" ]");console.log("equation.shadow -- "+e);return""}};for(var i=0;i<context_variables.length;++i)$('[name="'+context_variables[i].replace("$","")+'"]',$wrapper).on("input change",function(event,source){var passthrough=$(this).attr("data-equation-passthrough");if(source=="equation.event"&&passthrough!=="true")return;var result=update();if(result===
""&&$equation.val()!=""&&$equation.attr("data-equation-computed")!=="true")return;else{$equation.attr("data-equation-computed","true");$equation.val(result)}$equation.trigger("change","equation.event");$equation.trigger("input","equation.event")})});$("input, select, textarea",$wrapper).one().trigger("input")}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"innerform","wrapper":"[data-innerform]","defaults":{"api-target":"[data-innerform-action]"},"routines":{"refresh":function(conf,wrapper,self){}},"init":function(conf,wrapper,self){var $targets=$.jshadow.children(conf,wrapper,self,conf["api-target"]);$targets.each(function(){var $target=$(this),$form=null,$scope=$target.closest("[data-innerform]");if($target.attr("form")===undefined)$form=$target.closest("form");else $form=$("#"+$target.attr("form"));if($form===null){console.log("innerform.shadow: failed to locate form for ",
$target);return}$target.on("click",function(event){event.preventDefault();event.stopPropagation();$.xhrPool.abortAll();if($scope.attr("data-loading")!=="true"){$scope.showLoading();$scope.attr("data-loading","true")}$('[data-dismiss="modal"]',$scope).trigger("click");if(typeof mj==="undefined"||mj.browser.ie)$('[form="'+$form.attr("id")+'"]').not("form [form]").each(function(){var $element=$(this),name=$element.attr("name"),$duplicates=$('[name="'+name+'"]',$form);$duplicates.remove();$form.prepend($element)});
var destination=$scope.attr("data-innerform"),params=$scope.attr("data-innerform-params"),href=window.location;if(typeof params==="undefined")params="";else href=window.location.href.replace(/#.*/gi,"");$("#"+destination).load($form.attr("action")+" #"+destination+" > *",$form.serializeArray(),function(response,status,xhr){if(status=="error"){console.log("Error: "+xhr.status+" "+xhr.statusText);return}$scope.hideLoading();$scope.attr("data-loading","false");$("#"+destination).jshadow();$(window).trigger("resize")})})});
self.api.refresh()},"action":function(event,conf,wrapper,self){self.api.refresh()}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"saveme","wrapper":"[data-saveme]","defaults":{"api-target":"input, select, textarea","api-msg":"[data-saveme-msg]"},"routines":{"refresh":function(conf,wrapper,self){}},"init":function(conf,wrapper,self){var $msg_blocks=$.jshadow.children(conf,wrapper,self,conf["api-msg"]),$inputs=$.jshadow.children(conf,wrapper,self,conf["api-target"]);$msg_blocks.hide();$inputs.each(function(){var $input=$(this);var original_val=$input.val();$input.on("change",function(){if(original_val!==
$(this).val())$msg_blocks.fadeIn()})});self.api.refresh()},"action":function(event,conf,wrapper,self){self.api.refresh()}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"ui","wrapper":"[data-ui]","defaults":{"api-target":"[data-ui-show]","api-view":"[data-ui-view]"},"routines":{"refresh":function(conf,wrapper,self){var current_state=this.attr("data-ui"),children=$.jshadow.children(conf,wrapper,self,conf["api-view"]);children.each(function(){var states=$(this).attr("data-ui-view");states=states.trim().split(" ");if($.inArray(current_state,states)===-1)$(this).hide();else $(this).show()})}},"init":function(conf,wrapper,self){self.api.refresh()},
"action":function(event,conf,wrapper,self){wrapper.attr("data-ui",$(this).attr("data-ui-show"));self.api.refresh()}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"xlinker","wrapper":"[data-xlink-scope]","defaults":{"api-target":"[data-xlink]","api-trigger":"[data-xlink-click]"},"routines":{},"action":function(event,conf,wrapper,self){var xlinks=$.jshadow.children(conf,wrapper,self,conf["api-trigger"]),value=$(this).attr("data-xlink");$('[data-xlink-click="'+value+'"]:visible').not(xlinks).each(function(){$(this).trigger("click")})}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($,undefined){$.jshadow({"name":"xload","wrapper":"[data-xload-scope]","defaults":{"api-target":"a[href]","api-canceler":"[data-xload-exclude]","destination":"page"},"init":function(){window.onpopstate=function(e){if(e.state!==null)window.location=e.state.href}},"binding":function(conf,wrapper,definition){var elements;elements=$.jshadow.children(conf,wrapper,definition,conf["api-target"]).not(conf["api-canceler"]+" *",wrapper);elements.on(definition.trigger+"."+definition.name,function(event){$.proxy(definition.action,
this)(event,conf,wrapper,definition)});$.each(definition.routines,function(key,func){wrapper.on(key+"."+definition.name,func)});if(typeof definition.init!=="undefined")definition.init(conf,wrapper,definition)},"action":function(event,conf,wrapper,self){var $target,href,$main_loading_plane=$("#main-loading-plane");if(mj===undefined||mj.browser.ie==true)return;$target=$(this);href=$target.attr("href");if(href.match(/^#.*/gi))return;if($("#page").attr("data-xload")==="off")return;event.preventDefault();
$.xhrPool.abortAll();if($main_loading_plane.attr("data-loading")!=="true"){$main_loading_plane.showLoading();$main_loading_plane.attr("data-loading","true")}$("#"+conf.destination).load(href+" #"+conf.destination+" > *",function(response,status,xhr){if(status=="error"){console.log("Error: "+xhr.status+" "+xhr.statusText);return}if(xhr.status!=200){window.location=href;return}if(response.match(new RegExp('id="'+conf.destination+'"'))===null){window.location=href;return}$main_loading_plane.hideLoading();
$main_loading_plane.attr("data-loading","false");$("#"+conf.destination).jshadow();history.pushState({"href":href},"",href);$(window).trigger("resize")})}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"xref","wrapper":"[data-xref-scope]","defaults":{"api-target":"[data-xref-target]","api-ref":"[data-xref]"},"routines":{"refresh":function(conf,wrapper,self){}},"init":function(conf,wrapper,self){var $targets=$.jshadow.children(conf,wrapper,self,conf["api-target"]),$references=$(conf["api-ref"]);$targets.each(function(){var $target=$(this);$target.on("click.mjolnir_xref",function(event){$references.off("click.mjolnir_xref_activate");$references.on("click.mjolnir_xref_activate",
function(){$target.closest("form").submit()})})});self.api.refresh()},"action":function(event,conf,wrapper,self){self.api.refresh()}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"xselect","wrapper":"[data-xselect-scope]","defaults":{"api-source":"[data-xselect-source]","api-master":"[data-xselect-master]"},"init":function(conf,wrapper,self){var $slaves=$.jshadow.children(conf,wrapper,self,conf["api-source"]),$master=$.jshadow.children(conf,wrapper,self,conf["api-master"]);var groups={},slavecache={};var source_attr=conf["api-source"].replace(/(^\[|\]$)/gi,"");$slaves.each(function(){var $slave=$(this),grouptitle=$slave.attr(source_attr),groupname=
$slave.attr("name");slavecache[groupname]=$slave;$slave.hide();groups[groupname]={title:grouptitle,options:[]};$("option",$slave).each(function(){var $option=$(this);groups[groupname].options.push({key:$option.attr("value"),title:$option.text()})});groups[groupname].options.shift()});var markup="",blank=wrapper.attr("data-xselect-blank");if(blank!==null&&typeof blank!=="undefined")markup+='<option value="'+blank+'">&nbsp;</option>';else blank=null;$.each(groups,function(groupname,group){if(group.options.length>
0){markup+='<optgroup label="'+group.title+'">';$.each(group.options,function(idx,option){markup+='<option value="'+groupname+"--"+option.key+'">'+option.title+"</option>"});markup+="</optgroup>"}});$master.html(markup);$master.trigger("change");$master.on("change.mjolnir_xselect",function(event){var key=$master.val(),value=key.replace(/[a-zA-Z0-9-_]+\-\-/gi,""),group=key.replace(new RegExp("--"+value),"");$slaves.each(function(){var $slave=$(this);$slave.val(null)});slavecache[group].val(value);
$slaves.trigger("change")});$slaves.each(function(){var $slave=$(this),value=null;if((value=$slave.val())!==blank){$("[selected]",$master).removeAttr("selected");$('[value="'+$slave.attr("name")+"--"+value+'"]',$master).attr("selected","selected");return false}});$master.show()}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"xsync","wrapper":"[data-xsync-scope]","defaults":{"api-target":"[data-xsync]"},"routines":{"refresh":function(conf,wrapper,self){}},"init":function(conf,wrapper,self){var $targets=$.jshadow.children(conf,wrapper,self,conf["api-target"]);$targets.each(function(){var $target=$(this);$target.on("change.mjolnir_xsync",function(){var caller,blank,noblank,list,html,$slaves;caller=$target.attr("data-xsync-call");blank=$target.attr("data-xsync-blank");noblank=$target.attr("data-xsync-noblank");
$slaves=$($target.attr("data-xsync"));caller=eval(caller);list=caller($target.val());if(typeof noblank!=="undefined"&&noblank=="true")html="";else{if(typeof blank==="undefined")blank="&nbsp;";html='<option value="">'+blank+"</option>"}if(list!==null&&typeof list!=="undefined")if(list.length===0){$slaves.html(html);$slaves.attr("disabled","disabled");$slaves.trigger("change")}else{$.each(list,function(i,item){html+='<option value="'+item["id"]+'">'+item["title"]+"</option>"});$slaves.html(html);$slaves.removeAttr("disabled");
$slaves.trigger("change")}else console.log("xsync: invalid list recieved")})});self.api.refresh()},"action":function(event,conf,wrapper,self){self.api.refresh()}})})(window.jQuery);/*
 2012, Ibidem Team
 @license https://github.com/ibidem/ibidem/blob/master/LICENSE.md (BSD-2)
*/
(function($){$.jshadow({"name":"xtoggle","wrapper":"[data-xtoggle-scope]","defaults":{"api-target":"[data-xtoggle]"},"routines":{"refresh":function(conf,wrapper,self){}},"init":function(conf,wrapper,self){var $targets=$.jshadow.children(conf,wrapper,self,conf["api-target"]);$targets.each(function(){var $target=$(this);$target.on("change.mjolnir_xtoggle",function(){$($target.attr("data-xtoggle")).toggle()})});self.api.refresh()},"action":function(event,conf,wrapper,self){self.api.refresh()}})})(window.jQuery);
